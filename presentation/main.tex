...
\begin{frame}{Two view and 1D convolutional encoder}
  ...
  \begin{tikzpicture}[
    ...
    \node[lbl, below=0.15cm of e1] {ambient \emph{or} tangent};
...
\begin{frame}{Beyond accuracy: frozen-probe robustness (3-seed)}
  ...
  \begin{center}
    \includegraphics[width=1.1\textwidth]{robustness_3seed.png}  % 10% bigger
  \end{center}
  \vfill
...
\begin{frame}{Manifold viz generalises across sustained TUH tasks}
  ...
  \begin{columns}[c]
    ...
    \textbf{TUSZ -- 5 seizure classes; SIGReg-tangent separates best}\\[0.3em]
    ...
    \vspace{0.9em}
    \begin{itemize}
      \item \textbf{AIRM $\ge$ Euclidean} wherever there is cluster structure (TUAB, TUSZ) -- de Surrel's Riemannian metric sharpens the embedding.
      \item \textbf{SIGReg (ours) $>$ random-init} on every task, in both decodability and clustering.
    \end{itemize}
  \end{columns}
\end{frame}
...
\begin{frame}{Limitation: transient events don't cluster (TUEV)}
  \scriptsize
  \begin{columns}[c]
    \column{0.5\textwidth}\centering
    \textbf{TUEV -- 6 event types (decodable, not clustered)}\\[0.3em]
    \includegraphics[width=\textwidth]{riemann_latent_tuev_annot.png}
    \column{0.5\textwidth}
    \begin{itemize}
      \item The manifold viz clusters \textbf{sustained} phenomena (abnormality, seizures), \emph{not} \textbf{transient} events -- TUEV's sub-second events are diluted by the 10\,s window.
      ...
      \item At TUEV's silhouette ${\approx}0$ (Euc $-0.003$, AIRM $-0.004$), AIRM vs Euclidean is within noise.
    \end{itemize}
  \end{columns}
  \vspace{0.3em}
...
\begin{frame}{Conclusion}
  ...
  \begin{itemize}
    ...
    \item \textbf{The payoff is the latent geometry}: the frozen SPD latent is visibly \emph{organised} -- normal/abnormal and seizure classes form coherent structure -- and \textbf{de Surrel's AIRM Riemannian embedding sharpens it over the Euclidean view on every clustered task, most clearly for SIGReg}.
  \end{itemize}
  \vspace{1.0em}
  \begin{center}
