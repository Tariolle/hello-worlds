...
- **The principled object is already our 0.761 baseline.** `examples/eeg/baseline_riemann.py` (pyRiemann `Covariances` + `TangentSpace`) *is* channel-cov + AIRM-at-Fréchet-mean tangent + LR. Evaluated directly as a probe feature it scores **0.761 ≪ 0.82** — the very object the "principled fix" steers toward already *underperforms* ambient by ~6 points (the linear probe re-derives second-order tangent structure for free). Best case for tangent SSL is therefore a *tie*. (Caveat: the baseline uses *raw* channel covariances, not learned-feature covariances — an imperfect but sign-clear proxy.)
