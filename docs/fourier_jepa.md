# Fourier-JEPA — STFT spectral-stem encoder
...
## Architecture (`examples/eeg/fourier_encoder.py`)
...
`model.encoder.type: fourier`; the default `conv1d` keeps existing
configs/checkpoints on the conv path.
...
```bash
python -m pytest tests/test_fourier_encoder.py -q     # shape/grad/SPD-rank contract
python examples/eeg/smoke_fourier.py                  # fwd+bwd, all reg cells
```

On Dalia (from the isolated `$WORK/fourierjepa/hello-worlds` checkout):

```bash
sbatch cluster/smoke_fourier.sbatch                                              # validate graph on B200
sbatch cluster/train_fourier.sbatch                                             # Fourier SIGReg-ambient
CFG=examples/eeg/cfgs/train.yaml sbatch --export=ALL,CFG cluster/train_fourier.sbatch   # conv baseline
sbatch --dependency=afterok:<fourier_jobid>:<conv_jobid> cluster/bench_fourier.sbatch   # comparison table
```
